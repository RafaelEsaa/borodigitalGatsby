...
class Blog extends Component {
    render() {
        ...
        const posts = dataPosts.map((data, key) => {
            const imgPost = dataAllFile
                .map(value => {
                    if (
                        value.node.childImageSharp.fluid.src.match(
                            data.node.frontmatter.imgthumbnail
                        )
                    ) {
            ...
            <Layout location={location} menuFixed={false}>
                ...
                <div className="row full-width no-margin pb-4">
                    <div className="col-md-12">
                        <h1 className="text-center title-banner-post">
                            Últimos Posts
                        </h1>
                    </div>
                </div>
