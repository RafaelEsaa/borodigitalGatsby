...
import dataOurServices from '../data/services/servicesEsp.json'
...
const OurServices = ({ data }) => {
  ...
  const divOfServices = dataOurServices.services.map((data, i) => {
    const divImage = imageServices.map((valImage, k) => {
          ...
          return (
            <Link to={data.link}>
              <div key={i} className="flex-services">
                <img className="icon" src={valImage.node.childImageSharp.fluid.src} alt="icon-services" />
                <h2 className="text-center text-services no-margin">{data.title}</h2>
              </div>
            </Link>
          )
        } else {
  ...
  return (
    <div className="row full-width our-services no-margin" id="services">
      <div className="col-md-6 franja-radius-right">
        <h1 className="no-margin text-center title-section">{dataOurServices.title}</h1>
      </div>
...
export default props => {
  return (
    <StaticQuery
      query={graphql`
          query {
            imageServices: allFile(filter:{relativeDirectory:{eq: "images"}}){
              edges {
                node {
                  id
                  relativePath
