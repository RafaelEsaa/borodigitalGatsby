...
import dataMenu from "../data/menuEsp.json";
...
class Menu extends Component {
	...
	render() {
		...
		const optionsMenu = dataMenu.menu.map((value, key) => {
			if (value.id) {
				if (location.href == process.env.GATSBY_HOSTNAME + "es") {
					return (
				...
				} else if (location.href == process.env.GATSBY_HOSTNAME + "es/blog") {
					return (
						<li className="nav-item active no-margin" key={key}>
							<Link className="active font-menu" to={"/es/#" + value.id}>
								{value.title}
							</Link>
						</li>
					);
